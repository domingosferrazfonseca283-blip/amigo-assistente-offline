...
class AutonomyEngine:
    """Dá à Noémia espaço para escolher entre alternativas conhecidas, sem inventar capacidades."""
    ...
    def choose(self, goal: str, choices: list[Choice]) -> AutonomyDecision:
        ranked = sorted(choices, key=lambda item: item.score, reverse=True)
        ...
        rationale = "Escolha baseada nas alternativas, pontuações, razões e riscos disponíveis."
    ...
    def snapshot(self) -> dict[str, Any]:
        return {
            ...
            "decisions": [asdict(item) for item in self.decisions[-100:]],
        }
